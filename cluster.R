...
labs(title="LCD Age Profile Clusters (N=77)") +
  theme(axis.line.y=element_blank(), plot.subtitle=element_text(size=rel(0.78)), plot.title.position="plot",
        axis.text.y=element_text(colour="Black"),
        axis.text.x=element_text(size=12)) +
  theme(axis.text.y = element_text(size = 12)) +
...
jpeg("Outputs/cluster/ClusterLCDyellow.jpg", units="in", width=8, height=6, res=500)
plot(fig, align="h", rel_widths=c(1,0.2))
dev.off()
...
jpeg("Outputs/cluster/VizLcd.jpg", units="in", width=14, height=16, res=500)
